    ...
    <Card>
        <template #title>
            <div class="flex justify-between items-center border-b-2 pb-2">
                ...
                <Button type="button" label="Options" icon="pi pi-angle-down" @click="toggleMenu($event, index)" style="width: auto" />
            </div>
        </template>
        <hr width="100%" />
        <template #content>
            <div class="flex flex-col gap-2">
                <div class="flex flex-col gap-5 border-b-2 pb-4">
                    ...
                    <div class="flex justify-between">
                        <div>
                            <p class="text-[10px] uppercase text-gray-400">{{ formatCurrency(item?.buyyingPrice) }}</p>
                            <p class="font-medium" style="line-height: 0">50 kg</p>
                        </div>
                        <div>
...
const overlayMenuItems = ref([{ label: 'Save', icon: 'pi pi-save' }, { label: 'Update', icon: 'pi pi-refresh' }, { label: 'Delete', icon: 'pi pi-trash' }, { separator: true }, { label: 'Home', icon: 'pi pi-home' }]);
</script>
